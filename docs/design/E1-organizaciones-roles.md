...
## 10. Desviaciones aceptadas (sprint E1-fix, 2026-09-04)

Cambios en los que la implementación se aparta de este documento a sabiendas, con el motivo. Añadido al cerrar los hallazgos de `docs/design/E1-revision.md`.

| # | Desviación | Motivo |
|---|---|---|
| #25 | **Contrato de las actions de miembros por `userId`, no por `membershipId`** (§6.7 y `forms/memberships.ts`). `changeMemberRoleAction` y `removeMemberAction` reciben `userId` y localizan la membresía por el unique `(organizationId, userId)`. | El par `(organizationId, userId)` YA es único y la organización viene de la sesión, nunca del formulario: aceptar `membershipId` obligaría a comprobar que esa membresía pertenece a la organización activa (un `findUnique` más y una vía de ataque más). Con `userId` el selector no puede apuntar fuera del tenant por construcción. La UI no expone `membershipId` en ningún sitio. |
| #26 | **`leaveOrganizationAction()` sin parámetros**: siempre "salgo yo de la organización activa" (§6.7 preveía un `membershipId`). | Dar de baja a un tercero es `removeMemberAction` (ADMIN, con motivo). Mezclar ambos casos en una acción sin rol mínimo sería justo el error que la matriz de permisos evita. Se conserva el invariante «al menos un ADMIN»: el último administrador debe nombrar sucesor antes de salir. |
| #12 | **`20260904120200_e1_enforce` conserva seis `DROP INDEX` sin `IF EXISTS`.** | La migración ya está APLICADA en `erp` y `erp_test`. `CLAUDE.md` prohíbe editar una migración aplicada: cambiar el fichero altera su checksum y `prisma migrate deploy` aborta en cualquier entorno que ya la tuviera. El riesgo real (una base sin esos índices) sólo existiría en una instalación que no venga de TaxHacker; `tests/integration/migration.test.ts` demuestra que la cadena completa se aplica sin error sobre un volcado pre-E1 real. Se corregirá si alguna vez hace falta reconstruir el historial de migraciones. |
| #13 | **`20260904120100_e1_backfill` mantiene el slug `email + 6 hex`.** | Mismo motivo que #12 (migración aplicada). En RUNTIME sí está corregido: `buildOrganizationSlug()` usa el uuid completo, así que ninguna organización creada por la aplicación puede colisionar. El caso residual es un backfill sobre una base con dos usuarios cuyo prefijo de email Y los primeros 6 hex del uuid coincidan. |
| #22 | **Monedas duplicadas (organización + catálogo global)** no unificadas. | Es ruido de UI, no un problema de aislamiento. Se aborda en E2, junto al plan de cuentas y los impuestos, donde el catálogo de monedas se revisa entero. |
| #23 | **La cookie `taxhacker.active_org` sigue sin `iat` ni versión** (365 días). | La cookie es un HINT firmado, nunca una credencial: `requireOrg()` revalida la membresía contra la base de datos en cada request y una cookie de una organización que ya no le pertenece al usuario se ignora. Añadir `iat` mejoraría la higiene, no la autorización. |
| — | **Rate limit de invitaciones en memoria del proceso** (`lib/rate-limit.ts`). | No sobrevive a un reinicio ni se comparte entre réplicas. La barrera persistente es `Invitation.attempts`, que bloquea el enlace a los 5 intentos fallidos y sí vive en la base de datos. Un limitador distribuido (Redis) se plantea cuando haya más de una réplica. |
| — | **`tenantDb` abre una transacción por operación.** | Es lo que hace efectiva la barrera 2 (`SET LOCAL` de `app.current_org` / `app.current_user`) sin refactorizar los 32 ficheros heredados de golpe, que es justo lo que ADR-0007 quería evitar. Coste: un BEGIN/COMMIT por consulta. Cuando el código de negocio esté agrupado en `tenantTransaction` (E3), la envoltura por operación dejará de ser necesaria para las lecturas. |
