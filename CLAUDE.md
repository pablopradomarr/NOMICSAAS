...
- **Migraciones ejecutables por un rol NO superusuario** (Supabase: `postgres` tiene `rolsuper=false`, `rolcreaterole`, `rolbypassrls`): prohibido `ALTER ROLE … NOSUPERUSER`, `ALTER FUNCTION … OWNER TO` sin `GRANT CREATE ON SCHEMA` previo, y cualquier sentencia que exija SUPERUSER. Ver `docs/deploy/` y el runbook del preview (Project: DESPLIEGUE-PREVIEW.md).
