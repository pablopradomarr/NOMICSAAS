...
| `models/backups.ts`, `app/(app)/settings/backups` | Backup/restore JSON | **REESCRITO (E11, G-15)**. El bloque heredado se ha ido: cubría 9 tablas de 66 —ninguna contable—, `modelFromJSON` capturaba el error por fila **y lo sumaba igual a `insertedCount`**, `preprocessRowData` adivinaba tipos (la cuenta `0400` volvía `400`) y `REMOVE_EXISTING_DATA = true` borraba la organización de destino ANTES de leer si el archivo servía. En su lugar, **formato 2.0**: inventario derivado del esquema, JSONL con tipos EXPLÍCITOS, manifest canónico firmado con HMAC y `keyId` rotable, restauración **siempre a organización nueva** con aborto a la primera fila rechazada, y `DONE` sólo con las **seis comprobaciones** de §5.4. `cleanupOrganizationTables` —el único camino del código capaz de borrar asientos contabilizados— eliminado |
...
| `lib/stripe.ts`, `app/api/stripe/*` | Planes/membresía | **REESCRITO (E11)**: por ORGANIZACIÓN, contra un catálogo `Plan` versionado por vigencia, con webhook idempotente (`stripeEventId` UNIQUE) que **no crea tenants** —un cliente desconocido es `ORPHAN_WEBHOOK` con 200, no un alta— y `200` por defecto (el `400` de antes hacía que Stripe reintentara sin fin). En **modo INTERNO** (`BILLING_PROVIDER=none`, el defecto, ADR-0019 D9) las tres rutas responden **404** antes de tocar cliente, sesión o base |
| `lib/uploads.ts` → `lib/storage/*` | Ficheros en disco | **REESCRITO (E11, D-11)**: `StoredObject` + driver S3 (Supabase Storage) o local, clave por `sha256` con prefijo por organización. `lib/documents.ts` es el **único lector** de los bytes para las seis rutas que los leen |
...
- **G-15 backups** (MEDIA, el último abierto junto a G-14) → **CERRADO (E11)**: backup 2.0 firmado de todas las tablas de tenant, con el inventario derivado del esquema y las seis comprobaciones de §5.4 como condición de éxito. **G-20** (restaurar un ZIP ajeno desde la interfaz) queda fechado en **E12**: en E11 se restaura desde una copia que vive en la plataforma, que es el caso real, y un ZIP ajeno entra por script de operador.
