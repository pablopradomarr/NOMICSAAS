...
| `Project` (Proyecto directo) | Unidad con ingresos y gastos directos propios; pertenece a UNA línea de negocio | `code`, `name`, `businessLineId`, `customerId?`, `status` (`PLANNED/ACTIVE/CLOSED`), `startDate`, `endDate?`, `budgetRevenue?`, `budgetCost?` |
| `CostCenter` (CECO) | Centro de coste indirecto | `code`, `name`, `kind` (ver tipos), `allocatable` (bool) |
...
`INGRESO_DIRECTO` · `COSTE_DIRECTO_MC1` · `COSTE_DIRECTO_MC2` · `INDIRECTO_CECO` · `FINANCIERO` · `EXTRAORDINARIO` · `NO_ANALITICO`. Default por cuenta viene de `seeds/npgc.csv` (`tipo_analitico`) y es editable por organización.
...
|---|---|---|
| Ingresos | Σ `INGRESO_DIRECTO` del proyecto | 70x, 75x directos |
...
| **MC3** | MC2 − CECOs con `marginLevel = 3` imputados | OPERACIONES_INDIRECTAS, DESARROLLO_PRODUCTO (según config) |
| EBITDA | Σ MC3 − CECOs con `marginLevel = EBITDA` | MARKETING_VENTAS, G_A |
| EBIT | EBITDA − amortizaciones/deterioros (68x/69x/79x) | |
| BAI | EBIT − FINANCIERO ± EXTRAORDINARIO | 66x/76x, 67x/77x |
| Resultado | BAI − impuesto (630) | |

La PyG analítica se presenta por **proyecto**, agregada por **línea de negocio**, y en columna aparte los **CECOs no imputados** para que la suma total iguale la PyG contable (invariante I4).
...
| `targetKind` | `PROJECTS` / `BUSINESS_LINES` / `MIXED` |
...
4. Produce `AllocationLine {runId, ruleId, sourceCostCenterId, targetProjectId|targetBusinessLineId, amountCents, driverBase, driverShare}`. **No toca el libro diario**: es capa analítica paralela, reversible (`AllocationRun.reversedAt`).
